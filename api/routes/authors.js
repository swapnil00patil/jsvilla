...
router.post('/', function(req, res, next) {
  ...
  request = {
    ...
    unique_value: db.escape(request.name.trim().replace(/\s/g, '-').replace(/[^a-zA-Z0-9-]/g, "").toLowerCase()),
    description: db.escape(request.description.trim()) || '',
    twitter_url: db.escape(request.twitter_url.trim()) || '',
    github_url: db.escape(request.github_url.trim()) || ''
  }
